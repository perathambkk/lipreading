...
srun --mem=48GB  --ntasks=1 --gres=gpu:4 bash -c " 
      ...
      cd /vulcan/scratch/peratham/swpath/end-to-end-lipreading/shortfast_conv2_p_OSL_500_cosine;\
          time  python main.py --path 'temporalConv/temporalConv_11.pt' \
	 --dataset '/vulcan/scratch/peratham/lrw-repro/shf_npz_rgb' \
                                       	--mode 'temporalConv' --resume-epochs=12\
					--workers=16 --resume \
				--batch-size=84 --lr=${LR} \
                                       --epochs=40;
           " &
