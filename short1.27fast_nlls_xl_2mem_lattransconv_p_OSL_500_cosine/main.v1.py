...
def test_adam(args, use_gpu):
	...
	if args.mode == 'temporalConv' or args.mode == 'finetuneSelfAttention':
		...
		# for param in model.model.pathway0_backend_conv1.parameters():
		# 	param.requires_grad = True
		# for param in model.model.pathway1_backend_conv1.parameters():
		# 	param.requires_grad = True
		# for param in model.model.backend_conv2.parameters():
		# 	param.requires_grad = True
		...
		# 		non_bn_parameters.append(p)
		optim_params = [
			{"params": tbn_params, "weight_decay": 0.0, "betas":(0.9, 0.98)},
			{"params": tnon_bn_parameters, "weight_decay": 1e-4, "betas":(0.9, 0.98)},
		]
		optimizer = optim.Adam(optim_params, lr=args.lr, betas=(0.9, 0.999))
	else:
		raise Exception('No model is found!')
