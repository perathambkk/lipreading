...
def train_test(model, dset_loaders, criterion, epoch, phase, optimizer, scheduler, warmup_scheduler, args, logger, use_gpu, save_path):
	...
	slow_idx = torch.tensor(range(7,22)).long()
	min_scale = [128]
	max_scale = [160]
	crop_size = [112]
...
def test_adam(args, use_gpu):
	...
	# Non-memory parameters.
	non_mem_parameters = []
	for name, p in model.t_encoder2.named_parameters():
		if "memory" in name:
			mem_params.append(p)
		else:
			non_mem_parameters.append(p)
	...
	if args.mode == 'temporalConv' or args.mode == 'finetuneSelfAttention':
		...
		for param in model.t_encoder2.parameters():
			param.requires_grad = True
		for param in model.backend_tlinear.parameters():
			param.requires_grad = True
		...
		# 	else:
		# 		non_bn_parameters.append(p)
		for name, p in model.backend_tlinear.named_parameters():
			if "bn" in name:
				bn_params.append(p)
			else:
				non_bn_parameters.append(p)
		optim_params = [
			{"params": model.t_encoder1.parameters(), "weight_decay": 0.0, "betas":(0.9,0.98)},
			{"params": model.t_encoder2.parameters(), "weight_decay": 0.0, "betas":(0.9,0.98)},
			{"params": bn_params, "weight_decay": 0.0, "betas":(0.9,0.999)},
			{"params": non_bn_parameters, "weight_decay": 1e-4, "betas":(0.9,0.999)},
		]
		optimizer = optim.Adam(optim_params, lr=args.lr, betas=(0.9, 0.98), eps=1e-9)
	else:
		raise Exception('No model is found!')
