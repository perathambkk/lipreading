...
def main(args):
	...
	splits = ['test.txt', 'train.txt', 'val.txt']
	allcmds = []

	split = splits[args.sidx]

	filepath = os.path.join(LRS2_path, split)
	fp  = open(filepath, 'r')
	for vid_dir in fp:
		vid_dir = vid_dir.replace('\n','')
		vsrc = os.path.join(LRS2_main, '{}.mp4'.format(vid_dir))
		vdst = os.path.join(LRS2_frames, 'main', '{}'.format(vid_dir))
	...
	start = timer()
	num_batch = math.ceil(len(allcmds)/ float(n_workers * n_threads))
	for i in range(num_batch):
		start = i * n_workers * n_threads
		end = (i+1) * n_workers * n_threads
		if end > len(allcmds):
			end = len(allcmds)
		bcmds_all = allcmds[start:end]
		bcmds = []
		j = 0
