...
def mouth_roi(vtuple):
	im_path, detector, predictor, si, split, vi, fi = vtuple
	mouth_save_path = os.path.join(LRW_mouths_rois, si, split, '{}.yaml'.format(vi))
	...
	try:
		...
		return [mouth_t, mouth_b, mouth_l, mouth_r, mouth_centroid], None

		# mouth_crop_image = resized_img[mouth_t:mouth_b, mouth_l:mouth_r]
		# image = Image.fromarray(mouth_crop_image.astype('uint8'), 'RGB')
		# mouth_save_path = os.path.join(LRW_mouths, si, split, vi)
		# if not os.path.exists(mouth_save_path):
		# 	os.makedirs(mouth_save_path)
		# mouth_save_path = os.path.join(LRW_mouths, si, split, vi, fi)
	...
	for si in speaker_dirs_sub:
		for split in splits:
			...
			for vname in vid_dirs:
				vi_path = os.path.join(LRW_frames, si, split, vname)
				vframes = os.listdir(vi_path)
				vid_urls = [(os.path.join(vi_path, vf), detector, predictor, si, split, vname, vf) for vf in vframes]

				with ThreadPool(n_threads) as pool:
					start = timer()
					results = pool.imap_unordered(mouth_roi, vid_urls)
					
					rois = []
					for roi, error in results:
						if not roi:
							continue
					...
					roisavepath = os.path.join(LRW_mouths_rois, si, split)
					if not os.path.exists(roisavepath):
						os.makedirs(roisavepath)
					roisavepath = os.path.join(LRW_mouths_rois, si, split, '{}.yaml'.format(vname))
					with open(roisavepath, 'w') as fp:
						yaml.dump(rois, fp)
